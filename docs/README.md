# 文档目录

## 📁 文档结构

### 主要文档
- **[../README.md](../README.md)** - 项目主文档(快速开始、项目结构、开发状态)
- **[../开发指南.md](../开发指南.md)** - 详细的开发指引、代码示例、最佳实践
- **[../project-structure.md](../project-structure.md)** - 完整的架构设计和数据库设计

### 部署与运维
- **[QUICK_START.md](./QUICK_START.md)** - 10分钟快速安装指南
- **[DELIVERY_CHECKLIST.md](./DELIVERY_CHECKLIST.md)** - 交付清单
- **[DEPLOYMENT_CHECKLIST.md](./DEPLOYMENT_CHECKLIST.md)** - 部署清单

### 方法论文档
`methodology/` 目录包含业务方法论相关文档:
- 洞察提炼方法论
- 提炼实战工作表
- 测试效果评估
- 等等...

## 📖 阅读顺序

### 新手入门
1. **[../README.md](../README.md)** - 了解项目概况
2. **[QUICK_START.md](./QUICK_START.md)** - 快速安装配置
3. **[../开发指南.md](../开发指南.md)** - 开始开发

### 深入理解
1. **[../project-structure.md](../project-structure.md)** - 架构设计
2. **[../开发指南.md](../开发指南.md)** - 实现细节
3. `methodology/` - 业务方法论

### 部署上线
1. **[DEPLOYMENT_CHECKLIST.md](./DEPLOYMENT_CHECKLIST.md)** - 部署清单
2. **[DELIVERY_CHECKLIST.md](./DELIVERY_CHECKLIST.md)** - 交付清单

## 🎯 快速查找

| 你想要... | 看这个文档 |
|---------|----------|
| 快速启动项目 | [QUICK_START.md](./QUICK_START.md) |
| 开发新功能 | [../开发指南.md](../开发指南.md) |
| 了解架构设计 | [../project-structure.md](../project-structure.md) |
| 部署到生产 | [DEPLOYMENT_CHECKLIST.md](./DEPLOYMENT_CHECKLIST.md) |
| 查看 API | [../开发指南.md](../开发指南.md) + [../project-structure.md](../project-structure.md) |


